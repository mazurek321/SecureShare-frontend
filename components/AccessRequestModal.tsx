...
export default function AccessRequestModal({ isOpen, onClose, fileId, fileName, initialAlreadySent }: AccessRequestModalProps) {
  ...
  }, [isOpen, fileId, initialAlreadySent])
  ...
  const handleRequest = async () => {
    ...
    try {
      ...
      document.cookie = `sent_requests=${ids.join(',')}; max-age=31536000; path=/`
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-xl border border-white/10 bg-zinc-900 p-6 shadow-2xl">
        <h3 className="text-lg font-semibold text-white mb-2">Prośba o dostęp</h3>
        
        {success && (
          <div>
            <p className="text-zinc-300 text-sm mb-6">
              Twoja prośba o dostęp do pliku <span className="text-emerald-400 font-medium">{fileName}</span> została pomyślnie wysłana.
            </p>
        ...
        {isCached && !success && (
          <div>
            <div className="mb-4 rounded-xl bg-blue-500/10 p-3 text-sm text-blue-400 border border-blue-500/20 flex items-center gap-2">
              ...
              <span className="font-medium">Prośba została już wysłana.</span>
            </div>
            <p className="text-zinc-300 text-sm mb-6">
              Oczekuj na decyzję właściciela pliku <span className="text-white font-medium">{fileName}</span>. Nie musisz wysyłać jej ponownie.
            </p>
            <div className="flex justify-end">
        ...
        {!success && !isCached && (
          <div>
            <p className="text-zinc-300 text-sm mb-4">
              Nie posiadasz uprawnień do pobrania pliku <span className="text-white font-medium">{fileName}</span>. Czy chcesz wysłać prośbę o udostępnienie?
            </p>
